...
function hideChars(word: string): { display: string; hidden: number[] } {
  ...
  const count = Math.max(1, Math.floor(chars.length / 3));
  const indices: number[] = [];
  while (indices.length < count) {
    const i = Math.floor(Math.random() * chars.length);
    if (!indices.includes(i) && chars[i] !== ' ') indices.push(i);
  }
  indices.sort((a, b) => a - b);
  const display = chars.map((c, i) => indices.includes(i) ? '_' : c).join('');
  return { display, hidden: indices };
}
...
const FillBlanksGame = () => {
  ...
  const checkAnswer = async () => {
    ...
    setTimeout(async () => {
      if (currentIndex + 1 >= words.length) {
        ...
        await saveSession('fill-blanks', correctCount * 10, words.length, correctCount);
        setFinished(true);
      } else {
  ...
  if (finished) {
    ...
    return <GameResults score={correct * 10} total={words.length} correct={correct} gameType="fill-blanks" onPlayAgain={reset} />;
  }

  return (
    <div className="max-w-lg mx-auto">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-display font-bold text-foreground">Fill in Blanks ✏️</h1>
